<template>
	<view class="container">
		...
		<view class="user-section">
			<image class="bg" src="https://boyangchuanggu-mall.oss-cn-guangzhou.aliyuncs.com/static/user-bg.jpg">
			</image>
			<view class="user-info-box">
				<view class="portrait-box">
					<image class="portrait"
						:src="userInfo.icon || 'https://boyangchuanggu-mall.oss-cn-guangzhou.aliyuncs.com/static/missing-face.png'">
					</image>
				</view>
				<view class="info-box">
					<text class="username">{{ userInfo.nickname || (userInfo.phone_number ?
						formatPhone(userInfo.phone_number) : '游客') }}</text>
					<!-- #ifdef MP-WEIXIN -->
					<text class="phone" v-if="userInfo.phone_number && userInfo.nickname">{{
						formatPhone(userInfo.phone_number) }}</text>
					<!-- #endif -->
				</view>
			</view>
		...
		<view class="cover-container" :style="[{
			...
			<view class="history-section icon">
				<!-- #ifdef MP-WEIXIN -->
				<list-cell icon="icon-fenxiang2" iconColor="#FA436A" title="邀请提现"
					@eventClick="navTo('/pages/user/inviteStats')"></list-cell>
				<!-- #endif -->
