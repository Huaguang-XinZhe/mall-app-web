					...
					<view v-for="(item, index) in orderList" :key="index" class="order-item"
						...
						<view class="i-top b-b">
							...
							<text v-if="item.status === 3 || item.status === 4"
								class="del-btn yticon icon-iconfontshanchu1" @tap.stop="deleteOrder(item.id)"></text>
						</view>
						...
						<view class="action-box b-t" v-if="item.status == 2">
							<view class="action-btn" @tap.stop="handleCheckLogistics(item)">查看物流</view>
							<button class="action-btn recom" @tap.stop="handleReceiveOrder(item)">确认收货</button>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		// 处理查看物流
		handleCheckLogistics(order) {
			if (!order || !order.id || !order.deliverySn) {
				uni.showToast({
					title: '物流信息不完整',
					icon: 'none'
				});
			...
			// 调用公共的查看物流方法
			checkLogistics(order.id, order.deliverySn, goodsInfo)
				.catch(error => {
